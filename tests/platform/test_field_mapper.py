...
from boss_career_ops.platform.field_mapper import _parse_salary, BossFieldMapper
from boss_career_ops.platform.models import Job, ChatMessage, Contact


class TestParseSalary:
    def test_k_range(self):
        assert _parse_salary("20K-40K") == (20000, 40000, 12)

    def test_k_range_with_months(self):
        assert _parse_salary("20K-40K·16薪") == (20000, 40000, 16)

    def test_lowercase_k(self):
        assert _parse_salary("15k-25k") == (15000, 25000, 12)
    ...
    def test_negotiable(self):
        assert _parse_salary("面议") is None

    def test_negotiable_variant(self):
        assert _parse_salary("薪资面议") is None
    ...
    def test_13_salary(self):
        assert _parse_salary("10K-20K·13薪") == (10000, 20000, 13)

    def test_tilde_separator(self):
        assert _parse_salary("15K~30K") == (15000, 30000, 12)

    def test_em_dash_separator(self):
        assert _parse_salary("15K—30K") == (15000, 30000, 12)

    def test_spaces_around_k(self):
        assert _parse_salary("20 K - 40 K") == (20000, 40000, 12)
