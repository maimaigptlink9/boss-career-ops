...
def run_export(keyword: str, city: str, output: str, count: int, fmt: str):
    ...
    params = adapter.build_search_params(keyword, city, page_size=min(count, 50))
    try:
...
def _export_csv(jobs: list, path: Path):
    with open(path, "w", newline="", encoding="utf-8-sig") as f:
        ...
        for job in jobs:
            writer.writerow([
                _sanitize_csv_value(job.job_name),
                _sanitize_csv_value(job.company_name),
                _sanitize_csv_value(job.salary_desc),
                _sanitize_csv_value(job.city_name),
                _sanitize_csv_value(job.experience),
                _sanitize_csv_value(job.education),
            ])
...
def _export_html(jobs: list, path: Path):
    rows = ""
    for job in jobs:
        rows += f"<tr><td>{job.job_name}</td><td>{job.company_name}</td><td>{job.salary_desc}</td><td>{job.city_name}</td></tr>\n"
...
def _export_md(jobs: list, path: Path):
    lines = ["# 职位列表\n"]
    for job in jobs:
        lines.append(f"- **{job.job_name}** @ {job.company_name} | {job.salary_desc} | {job.city_name}")
