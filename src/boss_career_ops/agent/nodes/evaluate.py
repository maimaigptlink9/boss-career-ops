...
from boss_career_ops.agent.prompts import EVALUATE_SYSTEM, EVALUATE_USER
...
async def run(state: dict) -> dict:
    ...
    if not job_ids:
        return {"evaluation_results": {}, "errors": ["无待评估职位"]}
    ...
    for job_id in job_ids:
        try:
            ...
            if llm is not None:
                try:
                    system_msg = SystemMessage(content=EVALUATE_SYSTEM)
                    user_msg = HumanMessage(
                        content=EVALUATE_USER.format(
                            profile=profile_text, jd=jd_text, rag_context=rag_context
                        )
                    )
