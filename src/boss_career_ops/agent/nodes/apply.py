...
from boss_career_ops.agent.prompts import APPLY_SYSTEM, APPLY_USER, sanitize_input
...
async def run(state: dict) -> dict:
    ...
    if llm is not None:
        try:
            ...
            user_msg = HumanMessage(
                content=APPLY_USER.safe_substitute(
                    cv_summary=sanitize_input(cv_summary),
    ...
    try:
        ...
        if not greet_result.ok:
            errors.append(f"打招呼失败: {greet_result.error}")
    except Exception as e:
    ...
    try:
        ...
        if not apply_result.ok:
            errors.append(f"投递失败: {apply_result.error}")
    except Exception as e:
