...
from boss_career_ops.agent.prompts import RESUME_SYSTEM, RESUME_USER, sanitize_input
...
async def run(state: dict) -> dict:
    ...
    if not target_job_id:
        return {"resume_versions": {}, "next_action": "", "errors": ["无目标职位"]}

    job = job_details.get(target_job_id) or get_job_detail(target_job_id)
    if not job:
        return {"resume_versions": {}, "next_action": "", "errors": [f"职位 {target_job_id} 详情获取失败"]}

    cv = get_cv()
    if not cv:
        return {"resume_versions": {}, "next_action": "", "errors": ["简历内容为空"]}
    ...
    if llm is not None:
        try:
            ...
            user_msg = HumanMessage(
                content=RESUME_USER.safe_substitute(
    ...
    return {"resume_versions": resume_versions, "next_action": "", "errors": errors}
