...
def build_search_params(
    ...
    if education:
        edu_code = EDUCATION_MAP.get(education)
        if edu_code:
            params["education"] = edu_code
    if job_type:
        jt_code = JOB_TYPE_MAP.get(job_type)
        if jt_code:
            params["jobType"] = jt_code
    if scale:
        scale_code = SCALE_MAP.get(scale)
        if scale_code:
            params["scale"] = scale_code
    if finance:
        fin_code = FINANCE_MAP.get(finance)
        if fin_code:
            params["financeStage"] = fin_code
    if salary:
        sal_code = SALARY_MAP.get(salary)
        if sal_code:
...
def filter_by_welfare(jobs: list[dict], welfare_keywords: str) -> list[dict]:
    ...
    for job in jobs:
        ...
        if not labels:
            welfare_str = job.get("welfare", "")
            if welfare_str:
                labels = [w.strip() for w in welfare_str.replace("，", ",").split(",") if w.strip()]
        if all(any(kw in label for label in labels) for kw in keywords):
            result.append(job)
    return result
